...
TEST_F(RelationApplicationServiceTest, 获取所有好友申请时_单条记录应该完整映射FriendApplicationView的所有字段)
{
...
TEST_F(RelationApplicationServiceTest, 获取所有好友申请时_如果只有自己发送的好友申请时_应该能按照申请时间降序显示)
{
    EXPECT_CALL(*mockUserClient, getUsers(testing::_))
        .Times(1)
        .WillOnce(testing::Return(QHash<QString, sys::relation::port::UserInfo>{
...
TEST_F(RelationApplicationServiceTest, 获取所有好友申请时_如果只有别人发送的好友申请_应该能按照申请时间降序显示)
{
    EXPECT_CALL(*mockUserClient, getUsers(testing::_))
        .Times(1)
        .WillOnce(testing::Return(QHash<QString, sys::relation::port::UserInfo>{
...
TEST_F(RelationApplicationServiceTest, 获取所有好友申请时_如果同时有自己发送的和别人发送的好友申请_应该能按照申请时间降序显示_姓名栏和状态栏应该区别显示)
{
    EXPECT_CALL(*mockUserClient, getUsers(testing::_))
        .Times(1)
        .WillOnce(testing::Return(QHash<QString, sys::relation::port::UserInfo>{
