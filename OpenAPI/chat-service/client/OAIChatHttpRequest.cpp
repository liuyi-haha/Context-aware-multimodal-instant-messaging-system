...
OAIChatHttpRequestInput::OAIChatHttpRequestInput() {
    initialize();
}

OAIChatHttpRequestInput::OAIChatHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void OAIChatHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void OAIChatHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void OAIChatHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    OAIChatHttpFileElement file;
...
OAIChatHttpRequestWorker::OAIChatHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
OAIChatHttpRequestWorker::~OAIChatHttpRequestWorker() {
...
QMap<QString, QString> OAIChatHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

OAIChatHttpFileElement OAIChatHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return OAIChatHttpFileElement();
}

QByteArray *OAIChatHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void OAIChatHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void OAIChatHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void OAIChatHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void OAIChatHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  OAIChatHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString OAIChatHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void OAIChatHttpRequestWorker::execute(OAIChatHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<OAIChatHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, OAIChatCompressionType::Gzip));
        } else {
    ...
    if (OAIChatHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*OAIChatHttpRequestWorker::sslDefaultConfiguration);
    }
...
void OAIChatHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void OAIChatHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void OAIChatHttpRequestWorker::process_response(QNetworkReply *reply) {
    ...
    if (!contentDispositionHdr.isEmpty()) {
        ...
        if ((contentDisposition.count() > 0) && (contentDisposition.first() == QString("attachment"))) {
            ...
            OAIChatHttpFileElement felement;
...
QByteArray OAIChatHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray OAIChatHttpRequestWorker::compress(const QByteArray& input, int level, OAIChatCompressionType compressType) {
...
QSslConfiguration *OAIChatHttpRequestWorker::sslDefaultConfiguration;
